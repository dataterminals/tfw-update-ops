...
| `AllWeaponsUnlockableFix` | 🟨 | **Functionally OK, but rebuild is now required and its verifier will fail.** All 6 `EarlyAccess_*_Root` trees survive; the mod's own `mod_allowtags.json` (53 rownames) never touches the cut rows. **But** the deployed **Trees** variant's `trees_allowtags.json` has **56** rownames including `RFL01_Red/Blue/Green`, so the shipped pak overrides `WeaponsDetailsData` with a 56-row table that **re-adds 3 rows the devs deleted** — and those rows point at `BP_WPN_RFL01_VariantB/C` and `VT_Placeholder_Blue`, all **also deleted**. Nothing references the rows (mod's own research), so live risk is low, but it went from "harmless orphans" to "orphans with dangling asset pointers". `tools/verify_trees.py` hard-asserts `len==56`, `RFL01_* preserved`, and `31 widened` — all three now fail. **Fix at rebuild:** rebase onto the 53-row base, drop the 3 entries, update the verifier's constants (56→53, drop the presence check, 31→likely 28). |
