...
> `AllWeaponsUnlockableFix/tools/build_fix.sh` and `HeavyRifleRebalanceFix/tools/build_fix.sh` —
> the other two builds that pull mod-side assets out of a `zzz_`-staged mount. (HRF is 🟥 dead
> anyway; AWU is released and matters.)
>
...
> and so carry the same hazard un-gated — `HeavyRifleRebalanceFix`, `ScavgirlCarryPerks`,
> `TFWCharModelSelFramework`, `TFWQuestGiverPortraitPatch`. **Each needs auditing for a provenance
...
Last updated: 2026-08-01 — **`UnkillablesRebalanceFix` rebuilt and verified on `24501089`**
