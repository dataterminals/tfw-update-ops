...
| `AllWeaponsUnlockableFix` | 🟩 | **FIXED and rebuilt** (`3dafbc5`). Trees variant rebased onto the 53-row base: dropped the 3 `RFL01_*` entries from `trees_allowtags.json`, **inverted** the verifier's check to assert those rows are *absent* (so a future base re-adding them is caught), regenerated the verifier's stale 07-09 base dumps. `31 widened` held — those rows already had all 6 pawn tags, so were never in the widened set. **ALL CHECKS PASSED**: 5 roots correct, 0 null children, 0 dangling refs, 53 rows, 0 non-AllowTags changes, Gunhead not overridden. **Not deployed** — MO2 still holds the 07-20 pak. In-game test still outstanding. |
