...
| `AllWeaponsUnlockableFix` | 🟨 | **USER-CONFIRMED BREAK — root cause found, Trees fixed, regular rebuilding.** Both variants ship an overridden `WeaponsDetailsData` whose every row pins `DataAsset` → `DA_WPN_<code>_v2`; the patch renamed all of those to `DA_WPN_PLAYER_<code>`. Measured in a full mount: deployed Trees **56/56 rows dangling**, regular **56/56 dangling**, patched base 0/53, **rebuilt Trees 0/53** ✅. Every weapon loses its definition pointer → the customization UI breaks, exactly as reported on Nexus. See [`rootcause-awu-customization-ui.md`](rootcause-awu-customization-ui.md). Trees fixed + verified (`3dafbc5`); **neither variant deployed or released yet.** |
